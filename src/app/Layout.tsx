...
import {
  ...
  BarChart3, Camera, Settings, ChevronDown, Lightbulb
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="w-[230px] bg-[#E5DFCA] border-r border-[rgba(61,90,30,0.15)] min-h-screen flex flex-col fixed left-0 top-0 bottom-0 z-30">
        ...
        <div className="px-3 py-3 border-t border-[rgba(61,90,30,0.12)]">
          <button className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-lg hover:bg-[rgba(61,90,30,0.06)] transition-colors">
            <div className="w-8 h-8 bg-[#3D5A1E]/15 rounded-full flex items-center justify-center">
              <span className="text-[12px] text-[#3D5A1E] font-bold">OT</span>
            </div>
            <div className="flex-1 text-left">
              <p className="text-[12px] text-[#1A2412] font-semibold leading-tight">Operations Team</p>
              <p className="text-[10px] text-[#5C6B4A] leading-tight">Admin</p>
            </div>
            <ChevronDown className="w-3.5 h-3.5 text-[#5C6B4A]" />
          </button>
        </div>
      </aside>
